<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>后台首页</title>
  <meta name="robots" content="noindex, nofollow">
  <link href="/csqx/Public/Admin/css/general.css" rel="stylesheet" type="text/css" />
  <link href="/csqx/Public/Admin/css/main.css" rel="stylesheet" type="text/css" />
</head>
<body>
  <h1>
  <span class="action-span1"><a href="#">潮汕天气网</a></span>
  <div style="clear:both"></div>
  </h1>
  <ul id="lilist" style="padding:0; margin: 0; list-style-type:none; color: #CC0000;">
    </ul>
  <ul style="padding:0; margin: 0; list-style-type:none; color: #CC0000;">

  </ul>
  <div class="list-div">
  <table cellspacing='1' cellpadding='3'>
    <tr>
      <th colspan="4" class="group-title">运营数据</th>
    </tr>
    <tr>
      <td width="20%"><a href="#">今日访问:</a></td>
      <td width="30%"><strong style="color: red">4</strong></td>

      <td width="20%"><a href="#">今日新增会员:</a></td>
      <td width="30%"><strong style="color: red">2</strong></td>
    </tr>
    <tr>
      <td><a href="#">总访问量:</a></td>
      <td><strong>3</strong></td>
      <td><a href="#">总会员数:</a></td>

      <td><strong>3</strong></td>
    </tr>
    <tr>
      <td><a href="#">昨日访问:</a></td>
      <td><strong>2</strong></td>
      <td><a href="#">昨日新增会员:</a></td>
      <td><strong>0</strong></td>

    </tr>
  </table>
  </div>

  <br />

  <div class="list-div">
  <table cellspacing='1' cellpadding='3'>
    <tr>
      <th colspan="4" class="group-title">预警数据</th>
    </tr>
    <tr>

      <td width="20%">当前预警数:</td>
      <td width="30%"><strong style="color: red">20</strong></td>
      <td width="20%">历史预警数：</td>
      <td width="30%"><strong>2</strong></td>
    </tr>
    <tr>
      <td><a href="#">当前全域预警数:</a></td>

      <td><strong style="color: red">11</strong></td>
      <td><a href="#">历史全域预警数:</a></td>
      <td><strong>10</strong></td>
    </tr>
    <tr>
      <td><a href="#">当前县市预警数:</a></td>
      <td><strong  style="color: red">11</strong></td>

      <td><a href="#">历史县市预警数:</a></td>
      <td><strong>0</strong></td>
    </tr>
  </table>
  </div>
  <br />

  <div class="list-div">
  <table cellspacing='1' cellpadding='3'>
    <tr>
      <th colspan="8" class="group-title">天气概况</th>

    </tr>
    <tr>
      <td width="10%">最高气温:</td>
      <td width="15%"><strong>6</strong></td>
      <td width="10%">最低气温:</td>
      <td width="15%"><strong>2</strong></td>
      <td width="10%">最大降水量:</td>
      <td width="15%"><strong>1</strong></td>
      <td width="10%">最大风力:</td>
      <td width="15%"><strong>4</strong></td>
    </tr>

    <tr>

      <td>热带气旋个数:</td>
      <td><strong>6</strong></td>

    </tr>
  </table>
  </div>

  <br />

  <div class="list-div">
  <table cellspacing='1' cellpadding='3'>
    <tr>
      <th colspan="4" class="group-title">系统信息</th>

    </tr>
    <tr>
      <td width="20%">服务器操作系统:</td>
      <td width="30%">Linux</td>
      <td width="20%">Web 服务器:</td>
      <td width="30%">Apache/2.2.4 (Win32) PHP/7</td>
    </tr>

    <tr>
      <td>PHP 版本:</td>
      <td>5.2.4</td>
      <td>MySQL 版本:</td>
      <td>5.0.45-community-nt</td>
    </tr>
    <tr>

      <td>安全模式:</td>
      <td>否</td>
      <td>安全模式GID:</td>
      <td>否</td>
    </tr>
    <tr>
      <td>Socket 支持:</td>

      <td>是</td>
      <td>时区设置:</td>
      <td>PRC</td>
    </tr>
    <tr>
      <td>GD 版本:</td>
      <td>GD2 ( JPEG GIF PNG)</td>

      <td>Zlib 支持:</td>
      <td>是</td>
    </tr>
    <tr>
      <td>IP 库版本:</td>
      <td>20071024</td>
      <td>文件上传的最大大小:</td>

      <td>2M</td>
    </tr>
    <tr>
      <td>编码:</td>
      <td>UTF-8</td>
      <td></td>
      <td></td>
    </tr>
  </table>

  </div>


  <div id="footer">
  共执行 28 个查询，用时 0.148943 秒，Gzip 已禁用，内存占用 3.336 MB<br />
  版权所有 &copy; 2016-2017 Keven工作室，并保留所有权利。</div>

</body>
</html>